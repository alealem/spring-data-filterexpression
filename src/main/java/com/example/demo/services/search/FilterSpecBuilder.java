...
public final class FilterSpecBuilder {
  ...
  private static final Set<String> JSONB_COLUMNS = Set.of("body", "payload", "metadata");

  private static Predicate toPredicate(
      FilterExpression expr, Root<?> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
  ...
  private static Predicate buildValuePredicate(
      ValueExpression v, Root<?> root, CriteriaBuilder cb) {
    ...
    Predicate p =
        switch (op) {
          ...
          case "greater-or-equal" ->
              cb.greaterThanOrEqualTo(pathAsComparable(path), asComparable(raw));
          ...
          case "like" ->
              buildLike(cb, path.as(String.class), Objects.toString(raw, ""), v.caseSensitive());
  ...
  private static Predicate buildJsonValuePredicate(
      ValueExpression v, Root<?> root, CriteriaBuilder cb) {
    ...
    Predicate p =
        switch (op) {
          ...
          case "regexp" -> {
            Expression<Boolean> expr =
                cb.function(
                    "regexp", Boolean.class, jsonText, cb.literal(Objects.toString(raw, "")));
            yield cb.isTrue(expr);
          }
  ...
  private static Predicate buildValuesPredicate(
      ValuesExpression v, Root<?> root, CriteriaBuilder cb) {
  ...
  private static Predicate buildBetweenPredicate(
      BetweenExpression b, Root<?> root, CriteriaBuilder cb) {
    ...
    Predicate p =
        cb.between((Expression) pathAsComparable(path), (Comparable) from, (Comparable) to);

    return maybeNegate(b.exclude(), p, cb);
  }

  private static Predicate buildLike(
      CriteriaBuilder cb, Expression<String> expr, String pattern, Boolean caseSensitive) {
  ...
  private static Comparable asComparable(Object raw) {
    ...
    throw new IllegalArgumentException(
        "Value is not comparable: " + raw + " (" + raw.getClass() + ")");
  }

  // ---- Combo-selection mapping (example) ----
  private static Predicate buildComboSelectionPredicate(
      ComboSelectionExpression c, Root<?> root, CriteriaBuilder cb) {
    ...
    for (SingleValueRowDataDto row :
        Optional.ofNullable(v.singleIncludeValues()).orElse(List.of())) {
      include.add(mapSingleRow(row, path, cb, false));
    }
    for (SingleValueRowDataDto row :
        Optional.ofNullable(v.singleExcludeValues()).orElse(List.of())) {
    ...
    Predicate includeP =
        include.isEmpty() ? cb.conjunction() : cb.or(include.toArray(Predicate[]::new));

    Predicate excludeP =
        exclude.isEmpty() ? cb.conjunction() : cb.and(exclude.toArray(Predicate[]::new));

    return cb.and(includeP, excludeP);
  }

  private static Predicate mapSingleRow(
      SingleValueRowDataDto row, Path<?> path, CriteriaBuilder cb, boolean negate) {
    ...
    Predicate p =
        switch (op) {
          ...
          case "PATTERN" ->
              cb.like(path.as(String.class), Objects.toString(val, "")); // expects %/_
          ...
          case "GREATER_OR_EQUAL" ->
              cb.greaterThanOrEqualTo(pathAsComparable(path), asComparable(val));
  ...
  private static Predicate mapRangeRow(
      RangeRowDataDto row, Path<?> path, CriteriaBuilder cb, boolean negate) {
  ...
  private static Expression<String> resolveJsonTextPath(
      Root<?> root, CriteriaBuilder cb, FieldPathDto field) {
    List<String> segs = field == null ? List.of() : field.segments();
    ...
    Expression<String> expr =
        cb.function("jsonb_extract_path_text", String.class, args.toArray(new Expression[0]));
